...
$IsVerify = $json_data['IsVerify'];
//$author = $_COOKIE['auth1'];
//$auth_token = $_SERVER["HTTP_AUTHORIZATION"];

$result = getOtp($Username, $IsVerify, $auth_token);
$json_data = json_decode($result, 1);
echo $result;

function getOtp($Username, $IsVerify, $auth_token) {
    $url = "https://profile.g88.vin/api/profile/SendTeleSafe";
    $ch = curl_init();
    curl_setopt($ch,CURLOPT_URL,$url);
    curl_setopt($ch,CURLOPT_POST,true);
    curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
    curl_setopt($ch,CURLOPT_HEADER,0);
    $data = "{\"IsVerify\":$IsVerify,\"Username\":\"$Username\"}";
